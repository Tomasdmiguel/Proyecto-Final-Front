...
export const ContainerSedes = ({ sedes }: { sedes: ISede[] }) => {
  ...
  return (
    <div
      className={`w-full min-w-[60vw] max-w-[80vw] lg:min-w-[45vw] lg:w-[45vw] h-full font-Marko my-20 ${sport == 2 ? "bg-blue-400" : sport == 3 ? "bg-orange-500" : "bg-main"
        } p-6  shadow-xl rounded-lg space-y-4 flex flex-col items-center`}
    >
      <div className="flex flex-row w-full justify-around">
        <div
          className={`border-b-2 ease-in-out duration-100 p-2 hover:border-secundario ${sport == 1
            ? "border-secundario"
            : sport == 2
              ? "border-blue-400"
              : sport == 3
      ...
      <div
        className={`p-4 rounded-md flex flex-col justify-center items-center shadow-md space-y-4 text-terciario-white w-full ${sport == 1 || sport == 2 || sport == 3 ? "flex" : "hidden absolute"
          }`}
      >
        {sport == 1 ? (
