      ...
      {usuario?.turnos && usuario.turnos.length > 0 ? (
        <div className="flex flex-col gap-16 text-2xl">
          {usuario.turnos.map((turno: any, index: number) => (
            ...
            <div
              key={index}
              className="w-full max-h-[50vw] rounded-sm shadow-xl hover:bg-main hover:text-white ease-in-out duration-300 p-4 space-y-4">
              ...
              <div className="flex justify-end mt-4">
                <button
                  ...
                  className="bg-terciario text-white p-3 rounded-lg font-semibold hover:bg-red-600 duration-200 ease-in-out">
                  Cancelar Reserva
                </button>
              </div>
            </div>
